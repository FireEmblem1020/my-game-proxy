...
// 🛑 CHANGE THE TEXT INSIDE THE QUOTES BELOW TO YOUR CHOSEN PASSWORD
const PROXY_PASSWORD = "how-did-you-get-here";
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                /* Invisible trigger area at the top left of the page */
                #secretTrigger {
                    position: absolute;
                    top: 0;
                    left: 0;
                ...
                /* Game launcher layout (Only appears when unlocked) */
        ...
        <body id="pageBody">
            ...
            <!-- Actual Game Proxy Launcher -->
            <div id="launcherScreen" class="container hidden">
                <h2>Web Proxy Launcher</h2>
                <p>Enter any URL to load inside an unblocked window:</p>
                <input type="text" id="targetUrl" placeholder="https://example.com">
                <button onclick="launchProxy()">Launch Website</button>
            </div>
            ...
            <script>
                ...
                function launchProxy() {
                    ...
                    let proxiedUrl = window.location.origin + '/proxy?url=' + encodeURIComponent(url) + '&pwd=' + encodeURIComponent(savedPassword);
...
// 2. Strip security block headers
...
app.use('/proxy', (req, res, next) => {
    ...
    createProxyMiddleware({
        ...
        onError: (err, req, res) => {
            res.status(500).send('Proxy Error: Unable to fetch website.');
        }
    })(req, res, next);
});

app.listen(PORT, () => {
    console.log('Proxy site running');
});
